...
def extract_text(content_dict, text_type="full"):
    ...
    if text_type == "TA" or text_type == "title_abstract":
    ...
    elif text_type == "description_features":
        paragraphs = []
        features = []  # Assuming features are stored in keys starting with 'f' (adjust if needed)
        for key, value in content_dict.items():
            if key.startswith('p'):
                paragraphs.append(value)
            elif key.startswith('F'): 
                features.append(value)
        # Combine paragraphs and features before joining
        return " ".join(paragraphs + features)
    elif text_type == "full":
        all_text = []
        # Helper function to handle potential dict values
        def get_text(value):
            if isinstance(value, str):
                return value
            elif isinstance(value, dict):
                # Join all string values in the dictionary
        ...
        if "title" in content_dict:
            all_text.append(get_text(content_dict["title"]))
        if "pa01" in content_dict:
            all_text.append(get_text(content_dict["pa01"]))
        
        # Add claims and description, handling dict values
        for key, value in content_dict.items():
            if key not in ["title", "pa01"]:
                all_text.append(get_text(value))
...
def main():
    ...
    parser.add_argument('--text_type', type=str, default='TA',
                        choices=['TA', 'claims', 'description', 'full', 'tac1', 'description_features'],
