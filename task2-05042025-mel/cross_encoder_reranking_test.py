...
def main():
    parser = argparse.ArgumentParser(description='Re-rank patents using cross-encoder scoring (testing queries only)')
    ...
    parser.add_argument('--queries_list', type=str, default='test_queries.json',
                        help='Path to testing queries JSON file')
    ...
    print(f"Loading testing queries from {args.queries_list}...")
    queries_list = load_json_file(get_full_path(args.queries_list))
    print(f"Loaded {len(queries_list)} testing queries")
    ...
    # Filter pre-ranking to include only testing queries
    pre_ranking = {fan: docs for fan, docs in pre_ranking.items() if fan in queries_list}
    print(f"Filtered pre-ranking to {len(pre_ranking)} testing queries")
    ...
    print("Starting re-ranking process for testing queries...")
    re_ranked = {}
    missing_query_fans = []
    missing_doc_fans = {}
    ...
    for query_fan, pre_ranked_docs in tqdm(pre_ranking.items(), desc="Processing queries"):
        ...
        # Re-rank documents
        print(f"\nRe-ranking {len(doc_texts)} documents for testing query {query_fan}")
    ...
    print(f"Number of testing queries processed: {len(re_ranked)}")
